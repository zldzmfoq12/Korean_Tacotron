...
def p_post_cbhg(inputs, input_dim, is_training, depth):
	"""
	Args:
		inputs: input tensor
		input_dim: dimension of input tensor
...
def p_cbhg(inputs, input_lengths, is_training, scope, K, projections, depth):
    ...
    with tf.variable_scope(scope):
        with tf.variable_scope('p_conv_bank'):
            ...
            conv_outputs = tf.concat(
                [conv1d(inputs, k, 128, tf.nn.relu, is_training, 'p_conv1d_%d' % k) for k in range(1, K + 1)], #1D Convolution layers using multiple types of Convolution Kernel.
                axis=-1																						 #Iterate K with increasing filter size by 1.
            )# Convolution bank: concatenate on the last axis to stack channels from all convolutions
        ...
        # Two projection layers:
        proj1_output = conv1d(maxpool_output, 3, projections[0], tf.nn.relu, is_training, 'p_proj_1')#1st Conv1D projections
        proj2_output = conv1d(proj1_output, 3, projections[1], None, is_training, 'p_proj_2')#2nd Conv1D projections
        ...
        # 4-layer HighwayNet:
        for i in range(4):
            highway_input = highwaynet(highway_input, 'p_highway_%d' % (i + 1), half_depth) #make 4 Highway net layers
        rnn_input = highway_input
